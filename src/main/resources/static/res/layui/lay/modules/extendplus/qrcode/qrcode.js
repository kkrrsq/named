/**
 * @fileoverview - Using the 'QRCode for Javascript library' - Fixed dataset of
 *               'QRCode for Javascript library' for support full-spec. - this
	...
	(function() {
		...
		QRCodeModel.prototype = {
			...
			setupTypeNumber : function(test) {
				...
				for (var i = 0; i < 18; i++) {
					var mod = (!test && ((bits >> i) & 1) == 1);
					this.modules[i % 3 + this.moduleCount - 8 - 3][Math
							.floor(i / 3)] = mod;
				}
			},
		...
		QRRSBlock.RS_BLOCK_TABLE = [ [ 1, 26, 19 ], [ 1, 26, 16 ], [ 1, 26, 13 ],
				...
				[ 4, 25, 9 ], [ 1, 134, 108 ], [ 2, 67, 43 ],
				[ 2, 33, 15, 2, 34, 16 ], [ 2, 33, 11, 2, 34, 12 ], [ 2, 86, 68 ],
				[ 4, 43, 27 ], [ 4, 43, 19 ], [ 4, 43, 15 ], [ 2, 98, 78 ],
				[ 4, 49, 31 ], [ 2, 32, 14, 4, 33, 15 ], [ 4, 39, 13, 1, 40, 14 ],
				[ 2, 121, 97 ], [ 2, 60, 38, 2, 61, 39 ], [ 4, 40, 18, 2, 41, 19 ],
				[ 4, 40, 14, 2, 41, 15 ], [ 2, 146, 116 ],
		...
		var svgDrawer = (function() {
			...
			Drawing.prototype.draw = function(oQRCode) {
				...
				function makeSVG(tag, attrs) {
					var el = document.createElementNS('http://www.w3.org/2000/svg',
							tag);
					for ( var k in attrs)
				...
				for (var row = 0; row < nCount; row++) {
					for (var col = 0; col < nCount; col++) {
						if (oQRCode.isDark(row, col)) {
							...
							child.setAttributeNS("http://www.w3.org/1999/xlink",
									"href", "#template")
				...
				: !_isSupportCanvas() ? (function() {
					...
					Drawing.prototype.draw = function(oQRCode) {
						...
						for (var row = 0; row < nCount; row++) {
							...
							for (var col = 0; col < nCount; col++) {
								aHTML
										.push('<td style="border:0;border-collapse:collapse;padding:0;margin:0;width:'
												+ nWidth
												+ 'px;height:'
												+ nHeight
												+ 'px;background-color:'
												+ (oQRCode.isDark(row, col) ? _htOption.colorDark
					...
					return Drawing;
						...
						: (function() { // Drawing in Canvas
							function _onMakeImage() {
								this._elImage.src = this._elCanvas
										.toDataURL("image/png");
							...
							/**
							 * Check whether the user's browser supports Data URI or
							 * not
							 * 
							 * @private
							 * @param {Function}
							 ...
							 * @param {Function}
							 *            fFail Occurs if it doesn't support Data
							 *            URI
							 */
								...
								for (var row = 0; row < nCount; row++) {
									for (var col = 0; col < nCount; col++) {
										...
										_oContext.fillStyle = bIsDark ? _htOption.colorDark
												: _htOption.colorLight;
										_oContext.fillRect(nLeft, nTop, nWidth,
												nHeight);
	
										// 안티 앨리어싱 방지 처리
										_oContext.strokeRect(
												Math.floor(nLeft) + 0.5, Math
														.floor(nTop) + 0.5,
												nRoundedWidth, nRoundedHeight);
	
										_oContext.strokeRect(
												Math.ceil(nLeft) - 0.5, Math
														.ceil(nTop) - 0.5,
												nRoundedWidth, nRoundedHeight);
									}
								}
							...
							/**
							 * Make the image from Canvas if the browser supports
							 * Data URI.
							 */
							...
							Drawing.prototype.clear = function() {
								this._oContext
										.clearRect(0, 0, this._elCanvas.width,
		...
		function _getUTF8Length(sText) {
			var replacedText = encodeURI(sText).toString().replace(
					/\%[0-9a-fA-F]{2}/g, 'a');
		...
		QRCode.prototype.makeCode = function(sText) {
			this._oQRCode = new QRCodeModel(_getTypeNumber(sText,
					this._htOption.correctLevel), this._htOption.correctLevel);
